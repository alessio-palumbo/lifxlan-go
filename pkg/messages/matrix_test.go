...
func TestSetMatrixFrameAnimation(t *testing.T) {
	newNColors := func(n int) []packets.LightHsbk {
		s := make([]packets.LightHsbk, n)
		hue := rand.IntN(360)
		for i := range s {
	...
	testCases := map[string]struct {
		...
		d          time.Duration
		want       []*protocol.Message
		wantNext   *protocol.Message
	}{
		"less than 64 colors": {
			length:     1,
			width:      7,
			frames:     [][]packets.LightHsbk{lessThan64Slice},
			brightness: 50,
			d:          time.Millisecond,
			want: []*protocol.Message{
				protocol.NewMessage(&packets.TileSet64{
					TileIndex: 0, Length: 1, Rect: packets.TileBufferRect{FbIndex: 1, Width: 7, X: 0, Y: 0},
					Duration: 0, Colors: lessThan64Array,
				}),
			},
			wantNext: protocol.NewMessage(&packets.TileCopyFrameBuffer{
				TileIndex: 0, Length: 1, SrcFbIndex: 1, DstFbIndex: 0,
				Width: 7, Height: 5, Duration: 1,
			}),
		},
		"equal 64 colors": {
			length:     1,
			width:      8,
			frames:     [][]packets.LightHsbk{equal64Slice},
			brightness: 50,
			d:          time.Millisecond,
			want: []*protocol.Message{
				protocol.NewMessage(&packets.TileSet64{
					TileIndex: 0, Length: 1, Rect: packets.TileBufferRect{FbIndex: 1, Width: 8, X: 0, Y: 0},
					Duration: 0, Colors: equal64Array,
				}),
			},
			wantNext: protocol.NewMessage(&packets.TileCopyFrameBuffer{
				TileIndex: 0, Length: 1, SrcFbIndex: 1, DstFbIndex: 0,
				Width: 8, Height: 8, Duration: 1,
			}),
		},
		"greater than 64 colors": {
			length:     1,
			width:      16,
			frames:     [][]packets.LightHsbk{greaterThan64Slice},
			brightness: 50,
			d:          time.Millisecond,
			want: []*protocol.Message{
				protocol.NewMessage(&packets.TileSet64{
		...
		"greater than 64 colors (with partial message)": {
			length:     1,
			width:      16,
			frames:     [][]packets.LightHsbk{greaterThan64PartialSlice},
			brightness: 50,
			d:          time.Millisecond,
			want: []*protocol.Message{
				protocol.NewMessage(&packets.TileSet64{
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			got, next := SetMatrixFrameAnimation(tc.startIndex, tc.length, tc.width, tc.frames, tc.brightness, tc.d)
			assert.Equal(t, got, tc.want)
			assert.Equal(t, next(), tc.wantNext)
		})
	}
}
